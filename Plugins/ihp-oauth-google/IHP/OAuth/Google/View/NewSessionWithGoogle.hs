...
instance (Router.HasPath GoogleOAuthController) => View NewSessionWithGoogleView where
    html NewSessionWithGoogleView{..} =
        [hsx|
            <button class="btn btn-primary" id="connect-with-google" data-client-id={clientId}>
                Login with Google
            </button>
            <script>
            ...
            <form
                method="POST"
                action={GoogleConnectCallbackAction}
                id="new-session-with-google-form"
            >
