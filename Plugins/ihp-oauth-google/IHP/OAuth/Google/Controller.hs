...
newSessionWithGoogleAction ::
    ...
    , ?request :: Request
    , ?respond :: Respond
    , HasPath Google.GoogleOAuthController
    ) =>
    IO ()
...
googleConnectCallbackAction = do
    ...
    case jwt of
        ...
        Right (Jwt.Jws (_, claimsString)) -> do
            ...
            let googleUserId =
                    get #jwtSub standardClaims
                        |> fromMaybe (error "No google user id provided")
            maybeUser :: Maybe user <-
                Sessions.usersQueryBuilder @user
...
ensureIsNotLocked :: forall user. (?context :: ControllerContext, ?request :: Request, ?respond :: Respond, HasNewSessionUrl user, HasField "lockedAt" user (Maybe UTCTime)) => user -> IO ()
