...
newSessionWithGithubAction :: forall user. (?context :: ControllerContext, ?request :: Request, HasPath Github.GithubOAuthController) => IO ()
newSessionWithGithubAction = do
    state <- Github.initState
    let options =
            Github.AuthorizeOptions
...
githubConnectCallbackAction = do
    ...
    accessTokenResponse <-
        Github.requestGithubAccessToken
            Github.RequestAccessTokenOptions
                { clientId = githubOAuthConfig.clientId
...
githubOAuthScopeConfig =
    ?context.frameworkConfig.appConfig
        |> TMap.lookup @Github.GithubOAuthScopeConfig
        |> fromMaybe Github.GithubOAuthScopeConfig{scope = ["user:email"]}

ensureIsNotLocked :: forall user. (?context :: ControllerContext, ?request :: Request, HasNewSessionUrl user, HasField "lockedAt" user (Maybe UTCTime)) => user -> IO ()
